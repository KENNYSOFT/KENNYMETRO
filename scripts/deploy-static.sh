#!/bin/sh
# 화면(HTML, SVG)만 서버에 올린다. native 재빌드도, 컨테이너 재생성도 필요 없다.
#
# 앱은 볼륨 안의 static 디렉토리를 정적 리소스 위치로 먼저 보므로(application.yml 의
# spring.web.resources.static-locations) 파일을 덮어쓰는 것만으로 반영된다.
#
# 서버 코드와 함께 바뀐 화면은 이 경로로 올리지 않는다. 화면이 새 API 응답을 기대하면 이미지를
# 새로 받아야 한다.
#
# 사용법: ./scripts/deploy-static.sh [ssh호스트]
set -eu

HOST="${1:-${KENNYMETRO_SSH_HOST:-ampere}}"
REMOTE_DIR="${KENNYMETRO_REMOTE_STATIC:-~/.local/share/kennymetro/static}"
...
# tar 파이프로 보낸다. rsync 가 없는 환경에서도 동작하고 디렉토리 구조를 유지한다.
