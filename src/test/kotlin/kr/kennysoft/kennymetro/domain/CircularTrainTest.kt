...
class CircularTrainTest {
    ...
    fun `순환선은 종착역 자리가 아니라 updnLine 으로 방향을 정한다`() {
        ...
        // when & then
        assertEquals(Direction.DOWN, inner?.direction)
        assertEquals(Direction.UP, outer?.direction)
    }
    ...
    fun `순환선에는 노선 끝이 없어 단축 운행 표시를 하지 않는다`() {
        ...
        // when & then
        assertEquals(ServiceKind.NORMAL, train?.service)
    }

    @Test
    fun `지선으로 들어가는 열차는 본선 뷰에 그리지 않고 지선 뷰가 보여준다`() {
        ...
        // when & then
        assertNull(onLoop)
        assertEquals("성수", onBranch?.destination)
        assertEquals(Direction.UP, onBranch?.direction)
    }
